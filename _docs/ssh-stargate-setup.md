...
1. Terminal setup (<a onclick="toggle_display('terminal_mac')">Mac</a>, <a onclick="toggle_display('terminal_win')">Windows</a>)
    ...
    - I prefer [iTerm2](https://iterm2.com/) because it works well with my preferred terminal-mode editor, [micro](https://micro-editor.github.io/)
    </div>
...
## Step 3: ssh to medusa and run a docker container
1. **Do not do class work on the stargate host.** 
    ```sh
    YOUR_USF_USERNAME@stargate:~ $ ssh medusa
    YOUR_USF_USERNAME@medusa:~ $ clab
    ...
    YOUR_USF_USERNAME@51c6b23256da ~ $ 
    ```
